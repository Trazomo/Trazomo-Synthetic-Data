...
// ---------------------------------------------------------------------------
// Small-business cluster 3, receivables wave (SMB-17, SMB-18, SMB-19). The
...
// them. SMB-20, SMB-21, SMB-22 and SMB-30 join this block when wave two ships
// their bytes; the id lists below are the only thing that has to change.
//
// Every derived assertion, tie-out and cardinality for these three lives in
...
const SMB_C3_DETERMINISTIC = ["SMB-17", "SMB-18", "SMB-19"];
...
test("SMB-17, SMB-18, SMB-19: no processor, gateway, card network or bank product is named (rule R-MOCK)", () => {
...
test("SMB-17, SMB-18, SMB-19: no file carries a name canon seats, retires or freezes (rule R-ROLE)", () => {
  const names = canonPersonNames();
  assert.ok(names.size > 0, "canon/people.md parsed to no names, so this screen would pass on anything");
  for (const file of c3Files()) {
    for (const name of names) {
...
test("SMB-17, SMB-18, SMB-19: every minted id is namespaced, and the wave mints only cluster 3 classes (rule R-NS)", () => {
  const seen = new Map();
  for (const file of c3Files()) {
...
test("SMB-17, SMB-18, SMB-19: every client id is seated in canon or lies inside the reserved co-200 band", () => {
...
test("SMB-17, SMB-18, SMB-19: no file carries an em dash or an en dash", () => {
  for (const file of c3Files()) {
    assert.ok(!file.text.includes("\u2014"), `${file.id} carries an em dash (U+2014)`);
    assert.ok(!file.text.includes("\u2013"), `${file.id} carries an en dash (U+2013)`);
  }
