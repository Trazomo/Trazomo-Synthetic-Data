...
// CORE-01 Exhibit B.2 rather than shared with the generator, every internal
// party is re-resolved against the CORE-04 roster, and the OPS-04 join is read
// out of that artifact's own regenerated bytes rather than out of its builder,
// so the two generators cannot agree with each other by sharing a function.
...
const ops04Spec = specs.byId.get("OPS-04");
...
/**
 * The OPS-04 vendor-portal request, read out of that artifact's regenerated
 * bytes. Never the committed CSV, so a stale file on disk cannot make this pass,
 * and never OPS-04's builder, so the two generators cannot agree by construction.
 */
function vendorAccessRequest() {
  assert.ok(ops04Spec, "OPS-04 not found in specs/artifact-specs.yaml");
  const queue = csvTable(fileByPath(generateArtifact(ops04Spec, canon), "intake-requests.csv").content);
  const vendorRows = queue.rows.filter((r) => r.requester_type === "vendor");
  assert.equal(vendorRows.length, 1, `OPS-04 carries ${vendorRows.length} vendor requests, expected 1`);
  return vendorRows[0];
}
...
test("OPS-13 U7: the network access was handed over after the request that asked for it arrived", () => {
  const { rows } = log();
  const request = vendorAccessRequest();
  const handovers = rows.filter((r) => r.from_team === "IT & Security" && r.to_team === PROVIDER_TEAM);
  assert.equal(handovers.length, 1, `${handovers.length} handoffs run from IT & Security to the provider, expected 1`);
  assert.equal(rowClass(handovers[0]), "closed", "the network access handoff is not closed");
  assert.match(
    handovers[0].deliverable, /network access/i,
    "the IT & Security handoff to the provider is not the network access one"
  );
  assert.ok(
    handovers[0].sender_completed_at.slice(0, 10) > request.received_date,
    `the access was handed over ${handovers[0].sender_completed_at.slice(0, 10)}, `
    + `on or before the request arrived (${request.received_date})`
  );
  assert.equal(request.requester_name, PROVIDER_CONTACT, "the OPS-04 vendor request no longer names the provider contact");
  assert.equal(request.requester_org, PROVIDER_TEAM, "the OPS-04 vendor request no longer names the provider");
});
